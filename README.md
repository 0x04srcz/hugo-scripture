...
[POSSE](https://indieweb.org/POSSE) is publishing on your own site and syndicating copies elsewhere. The theme supports it from both ends without prescribing how you post.

**Manifest.** Enable the `syndication` output format and the site publishes `/syndication.json`: every note, with its id, permalink, date, tags, and body as plain text.

```toml
[outputFormats.syndication]
  mediaType = 'application/json'
  baseName = 'syndication'
  isPlainText = true
  notAlternative = true

[outputs]
  home = ['html', 'rss', 'syndication']
```

The body is produced by the same partial as the site's own share links, so text posted by hand and text posted by a script are byte-identical. The `id` is the leading Denote timestamp of the filename (`20260416T114051`) when there is one, otherwise the permalink — it survives retitling, which a permalink does not, so it is safe as an idempotency key.

**Backlink.** Drop a `data/syndication.json` keyed by that same id, and each note renders its "also on" link in the discussion section:

```json
{
  "20260416T114051": {
    "mastodon_url": "https://mastodon.social/@you/115610844258140241",
    "bsky_url": "https://bsky.app/profile/you/post/3m6hkxbzqc72m"
  }
}
```

Per-page front matter (`mastodon_url`, `bsky_url`) still wins where present. The data file matters for content generated by a tool such as ox-hugo, where writing back into front matter would be overwritten on the next export.
